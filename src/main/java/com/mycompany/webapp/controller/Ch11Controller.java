...
public class Ch11Controller {
	...
	// DTO 객체(Command Object)와 폼 연결 select - 기본값 제공
	@GetMapping("/form2")
	...
	public String handleForm3(@ModelAttribute("member") Ch11Member member) {
		...
		if (member.getMskill() != null) {
			logger.info("skills: " + Arrays.toString(member.getMskill()));
		}

		return ("redirect:/ch11/content");
	}

	// DTO 객체(Command Object)와 폼 연결 - 기본값 제공(라디오 버튼)
	@GetMapping("/form4")
