...
public class Ch10Controller {
	...
	// 사용자 정의 클래스를 이용한 예외처리
	@RequestMapping("/handlingException5")
	public String handlingException5() // throws Exception : RuntimeException이 아닌 일반 Exception이면 throws Exception
	{
		logger.info("실행");

		int stock = 0;
		if (stock == 0) {
			throw new Ch10SoldOutException(); // 품절
			// throw new Ch10SoldOutException("상품 재고가 없음"); // 상품 재고가 없음
		}
